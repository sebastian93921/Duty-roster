...
function genCal(month){
  axios.get(window.location.origin+"/site-config.json").then((response) => {
    ...
    axios.get(response.data.currentGooglePathCsv + response.data.genCsvExtension + month).then((response) => {
      genDutyByResponse(response);
    });

    if(!response.data.nextGooglePathCsv && response.data.currentGooglePathCsv === response.data.nextGooglePathCsv){
      return;
    }

    axios.get(response.data.nextGooglePathCsv + response.data.genCsvExtension + month).then((response) => {
      genDutyByResponse(response);
    });
  });
}

function genDutyByResponse(response){
